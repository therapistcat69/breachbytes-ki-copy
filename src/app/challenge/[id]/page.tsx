...
const ChallengePage = ({ params }: { params: any }) => {
  // params may be a Promise in some Next.js versions — unwrap safely
  ...
  // Fetch challenge data when the component mounts or challengeId resolves
  useEffect(() => {
  ...
  const handleSubmit = () => {
    if (triesLeft > 0) {
      ...
      setTriesLeft((n) => n - 1);
      // add answer-check logic here
    }
  };

  const getTriesCounterClass = () => {
    if (!challenge) return "";
    if (triesLeft === 0) return "danger";
    if (triesLeft < challenge.maxTries) return "warning";
    return "";
  };

  const renderClickableChest = () => {
    if (isLoading) return <div className="challenge-status-indicator">Loading Challenge...</div>;
    if (error) return <div className="challenge-status-indicator error">{error}</div>;
    return (
      <div className="clickable-svg-wrapper" onClick={togglePopup} role="button" tabIndex={0}>
        <Image src="/clickable-chest.svg" alt="Click to open challenge" width={150} height={150} />
      </div>
    );
  };
  ...
  return (
    <div className="challenge-container">
      <div className={`background-content ${isPopupVisible ? "content-blurred" : ""}`}>
        <Image src="/top-view-map.svg" alt="Challenge Map" layout="fill" objectFit="cover" className="background-svg" />

        {renderClickableChest()}
      </div>

      {isPopupVisible && challenge && <TreasureChest points={challenge.points || 250} players={challenge.playersSolved || 69} />}
          ...
          <div className="popup-content">
            ...
            <div className="popup-question-area">
              {challenge.questionType === "text" ? (
                ...
                <div className="popup-question-content">
                  <Image src={challenge.questionContent} alt="Challenge Image" width={400} height={300} className="question-image" />
            ...
            <div className="popup-answer-area">
              ...
              <div className="popup-footer">
                {challenge.attachment && (
                  <a href={challenge.attachment} download className="popup-attachment-button" aria-label="Download attachment">
                    <span className="btn-icon">📎</span>
                    <span className="btn-label">Attachment</span>
                  </a>
                )}

                <button className="popup-submit-button" onClick={handleSubmit} disabled={triesLeft === 0} aria-disabled={triesLeft === 0} aria-label="Submit answer">
                  <span className="btn-icon">✔</span>
